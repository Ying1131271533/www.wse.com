...
/*
    ...
    使用方法：
    // 1.实例化Hashids类
    $hashids = Hashids::instance(6, 'mackie'); // 6为邀请码的长度，mackie为加盐**
    $hashids = Hashids::instance(6); // 不填盐的话，就会使用默认的盐
    // 2.根据用户Id加密
    $invitation_code = $hashids->encode(5467);
    // 3.反向解密
    $id = $hashids->decode('lXNqgV');
*/
...
class Hashids implements HashGenerator {
    ...
    const hashids_salt = 'Akali-Ying';        // 加**匙
